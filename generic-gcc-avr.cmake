...
function(add_avr_executable EXECUTABLE_NAME)
   ...
   add_custom_target(
      ...
      ${AVR_UPLOADTOOL} -p ${AVR_UPLOADTOOL_MCU} -c ${AVR_PROGRAMMER} ${AVR_UPLOADTOOL_OPTIONS}
         -U flash:w:${hex_file}
      DEPENDS ${hex_file}
      COMMENT "Uploading ${hex_file} to ${AVR_UPLOADTOOL_MCU} using ${AVR_PROGRAMMER}"
   )
   ...
   add_custom_target(
      ...
      COMMENT "Uploading ${eeprom_image} to ${AVR_UPLOADTOOL_MCU} using ${AVR_PROGRAMMER}"
   )

   # get status
   add_custom_target(
      get_status
      ${AVR_UPLOADTOOL} -p ${AVR_UPLOADTOOL_MCU} -c ${AVR_PROGRAMMER} ${AVR_UPLOADTOOL_OPTIONS} -n -v
      COMMENT "Get status from ${AVR_UPLOADTOOL_MCU}"
   )

   # get fuses
   add_custom_target(
      get_fuses
      ${AVR_UPLOADTOOL} -p ${AVR_UPLOADTOOL_MCU} -c ${AVR_PROGRAMMER} ${AVR_UPLOADTOOL_OPTIONS} -n
         -U lfuse:r:-:b
         -U hfuse:r:-:b
      COMMENT "Get fuses from ${AVR_UPLOADTOOL_MCU}"
   )
   ...
   add_custom_target(
      get_calibration
         ${AVR_UPLOADTOOL} -p ${AVR_UPLOADTOOL_MCU} -c ${AVR_PROGRAMMER} ${AVR_UPLOADTOOL_OPTIONS}
         -U calibration:r:${AVR_UPLOADTOOL_MCU}_calib.tmp:r
         COMMENT "Write calibration status of internal oscillator to ${AVR_UPLOADTOOL_MCU}_calib.tmp."
   )
   ...
   add_custom_target(
      set_calibration
      ${AVR_UPLOADTOOL} -p ${AVR_UPLOADTOOL_MCU} -c ${AVR_PROGRAMMER} ${AVR_UPLOADTOOL_OPTIONS}
         -U calibration:w:${AVR_UPLOADTOOL_MCU}_calib.hex
         COMMENT "Program calibration status of internal oscillator from ${AVR_UPLOADTOOL_MCU}_calib.hex."
   )
